...
/// Integrations that submit ordered account calls can reserve `ADMIN_LANE` instead
/// of falling back to key 0. The demo's admin scripts use ordinary EVM transactions
/// and do not currently submit UserOperations on this lane.
///
...
contract LaneAccount is Simple7702Account {
    /// @dev Available to integrations for operations that must stay strictly ordered
    ///      relative to each other, kept away from the trading lanes.
